...
export class AdRecord implements AdEntity {
    ...
    static async getOne(id: string): Promise<AdRecord | null> {
        const [results] = await pool.execute("SELECT * FROM `adverts` WHERE id = :id", {
            id,
        }) as AdRecordResults;
    ...
    static async findAll(name: string): Promise<SimpleAdEntity[]> {
       const [results] = await pool.execute("SELECT * FROM `adverts` WHERE `name` LIKE :search", {
            search: `%${name}%`,
        }) as AdRecordResults;
    ...
    async insert(): Promise<void> {
        ...
        await pool.execute("INSERT INTO `adverts`(`id`, `name`, `description`, `price`, `url`, `lat`, `lon`) VALUES (:id, :name, :description, :price, :url, :lat, :lon)", this);
    }
}